...
router.get('/', (req, res)=>{
  ...
  conexion.query('SELECT * FROM user_login ',(error, results)=>{
      if (error) {
          throw error;
      }else{
...
router.get('/edit/:user_id', (req, res)=>{
  ...
  conexion.query('SELECT * FROM user_login WHERE user_id=?',[user_id], (error, results)=>{
      if (error) {
          throw error;
      }else{
...
router.get('/delete/:user_id', (req, res)=>{
  const user_id = req.params.user_id;
  conexion.query('DELETE FROM user_login WHERE user_id= ?', [user_id], (error, results)=>{
      if (error) {
          throw error;
      }else{
          res.redirect('/')
